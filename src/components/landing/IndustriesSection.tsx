      ...
      <StaggerContainer className="grid md:grid-cols-3 gap-6 lg:gap-8" staggerDelay={0.15}>
        {INDUSTRIES.map((ind) => (
          <StaggerItem key={ind.title}>
            <article className="group h-full rounded-2xl border border-border bg-card p-8 shadow-sm transition-all duration-300 hover:shadow-md hover:border-accent/30">
              <div className="mb-6 flex items-center justify-center w-14 h-14 rounded-2xl bg-accent/10 text-accent">
                {ind.icon}
              </div>
